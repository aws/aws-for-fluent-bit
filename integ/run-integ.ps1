...
Function Run-Test {
    ...
    # Find the parent folder name of the docker-compose config.
    # This would be used as the project name by default.
    $projectName = (Get-Item $DockerComposeTestFilePath).Directory.Name

    # Create temp folder to hold all the logs.
    $tempFolderPath = Join-Path C:\ $(New-Guid)
    New-Item -Type Directory -Path $tempFolderPath | Out-Null

    # Create a background process which would store the logs of docker-compose containers into log files.
    $logFile = Join-Path $tempFolderPath "run_${PluginUnderTest}.log"
    Start-Job -ScriptBlock {
        docker-compose -p $using:projectName logs -t -f > $using:logFile
    }
    ...
    # Before exiting, we will dump the logs from files so that CloudWatch can capture the same.
    Write-Host "Logs generated by containers during RunTest for ${PluginUnderTest} plugin are:"
...
Function Validate-Test {
    ...
    # Create a background process which would store the logs of docker-compose containers into log files.
    $logFile = Join-Path $tempFolderPath "validate_${PluginUnderTest}.log"
    Start-Job -ScriptBlock {
        docker-compose -p $using:projectName logs -t -f > $using:logFile
    }
    ...
    # Before validation, we will dump the logs from files so that CloudWatch can capture the same.
    Write-Host "Logs generated by containers during validation test for ${PluginUnderTest} plugin are:"
    Get-Content -Path $logFile
