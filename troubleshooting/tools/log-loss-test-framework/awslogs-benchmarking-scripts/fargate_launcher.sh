...
for SIZE_IN_KB in ${LOG_SIZES}; do
    ...
    for MAX_BUFFER_SIZE in ${BUFFER_SIZES}; do
        ...
        for test_throughput in ${THROUGHPUTS}; do
            ...
            export TEST_NAME="$THROUGHPUT"KB_"${SIZE_IN_KB}"KB_AWSLOGS_"${MAX_BUFFER_SIZE}"_BUFFER_FARGATE_AUTO
            TASK_DEF=$(envsubst < "$TASK_DEF_FILE")
            echo "$TASK_DEF" >> $TASK_DEF_OUTPUT_FILE
            TASK_DEF_ARN=$(aws ecs register-task-definition --region ${REGION} --cli-input-json  "$TASK_DEF"  | jq '.taskDefinition.taskDefinitionArn')
